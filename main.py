...
TOKEN = '8579040508:AAGXk-GFfzHa7DXfIyq4cKUMqcVQjSuxvGo'
...
def send_force_sub(chat_id, user_id):
    ...
    try:
        c.execute("SELECT link, style FROM channels")
    except:
        c.execute("SELECT link, 'primary' as style FROM channels")
    ...
    for i, ch in enumerate(channels):
        link = ch[0]
        btn_style = ch[1] if len(ch) > 1 and ch[1] else 'primary'
        row.append(InlineKeyboardButton(f"Join Channel {i+1}", url=link, style=btn_style))
...
def text_commands(message):
    ...
    # Agar user ne channel join nahi kiya aur direct button daba diya toh bot block karega aur menu hata dega!
    if not check_user_status(uid):
        # Keyboard hataane ke liye empty reply_markup bhejna
        ...
        # Wapas se force sub message bhejna
        send_force_sub(message.chat.id, uid)
        return
    # ----------------------------------------------------
